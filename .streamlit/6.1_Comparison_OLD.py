...
# Note: Column names from Section 3.0 analysis use "integrated_signal_" prefix
EXPECTED_INDIVIDUAL_COLS = {"wavenumber", "integrated_signal_withoutIR", "integrated_signal_withIR", "depletion"}
EXPECTED_SUMMED_COLS = {"wavenumber", "integrated_signal_withoutIR", "integrated_signal_withIR", "depletion"}
...
if nav_option == "Data Upload":
    ...
    st.write("Upload your CSV files below. For individual scans, ensure the file includes columns: **wavenumber**, **integrated_signal_withoutIR**, **integrated_signal_withIR**, and **depletion**.")
    ...
    if st.session_state.individual_files is None:
        ...
        try:
            with st.spinner("Aggregating individual scan data..."):
                ...
                # Aggregate data: compute averages for integrated_signal_withoutIR, integrated_signal_withIR, and depletion
                avg_df = (
                    big_df.groupby("wavenumber", as_index=False)
                    .agg({
                        "integrated_signal_withoutIR": "mean",
                        "integrated_signal_withIR": "mean",
                        "depletion": "mean"
                    })
                )
                avg_df.rename(
                    columns={
                        "integrated_signal_withoutIR": "integrated_signal_withoutIR_avg",
                        "integrated_signal_withIR": "integrated_signal_withIR_avg",
                        "depletion": "depletion_method2"
                    },
                    inplace=True
                )
                # Method 1: ratio of averaged values
                avg_df["depletion_method1"] = avg_df["integrated_signal_withIR_avg"] / avg_df["integrated_signal_withoutIR_avg"]
    ...
    if st.session_state.summed_file is not None:
        try:
            with st.spinner("Processing summed CSV data..."):
                ...
                sum_df.rename(
                    columns={
                        "integrated_signal_withoutIR": "integrated_signal_withoutIR_sum",
                        "integrated_signal_withIR": "integrated_signal_withIR_sum",
                ...
                if st.session_state.avg_df is not None:
                    compare_df = pd.merge(
                        ...
                        sum_df[["wavenumber", "integrated_signal_withoutIR_sum", "integrated_signal_withIR_sum", "depletion_method3"]],
                        on="wavenumber",
                        how="outer"
                    )
        ...
        try:
            ...
            with st.spinner("Generating static plot..."):
                ...
                ax.plot(st.session_state.compare_df["wavenumber"], st.session_state.compare_df["depletion_method1"],
                        label="Method 1; (integrated_signal_withIR_avg / integrated_signal_withoutIR_avg)", color=colors[3])
...
# ------------------------ Multi-file Comparison Section ------------------------
    ...
    st.write("Upload multiple CSV files to compare depletion spectra. Files should include columns: **wavenumber**, **depletion**, and/or **-ln(depletion)**.")
    
    multi_files = st.file_uploader(
        "Upload CSV files (depletion output from Section 3.0)",
        type=["csv"],
        accept_multiple_files=True,
        ...
        try:
            ...
            for file in multi_files:
                ...
                if "wavenumber" not in df.columns:
                    raise ValueError(f"{file.name} is missing required column: wavenumber")
                
                df.sort_values("wavenumber", inplace=True)
                df["filename"] = os.path.basename(file.name)
                all_data.append(df)
                
                # Track available columns across all files
            ...
            if not plot_columns:
                ...
                # Let user select which column(s) to plot
                ...
                if selected_columns:
                    ...
                    for col in selected_columns:
                        st.markdown(f"### {col.replace('_', ' ').title()} Comparison")
                        ...
                        fig = px.line(
                            ...
                            y=col,
                            color="filename",
                            title=f"Comparison of {col} by File",
                            labels={
                        ...
                        ax.set_title(f"Comparison: {col} vs. Wavenumber", fontsize=14, fontweight='bold')
